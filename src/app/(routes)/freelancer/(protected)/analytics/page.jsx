...
export default async function FreelancerAnalyticsPage() {
  ...
  const {rows: id} = await db.query('SELECT id from fm_freelancers WHERE clerk_id = $1',[
    userId
  ])
  const freelancer_id = id[0].id;
  // Get job_id's from fm_contracts using id as freelancer_id
  const {rows:job_idsArr} = await db.query('SELECT job_id FROM fm_contracts WHERE freelancer_id = $1',[
    freelancer_id
  ])
  ...
  let {rows: expensesArr} = await db.query('SELECT price FROM fm_expenses WHERE job_id = ANY($1)',[
      job_ids
    ]);
  ...
  let {rows: incomeArr} = await db.query('SELECT amount FROM fm_income WHERE job_id = ANY($1)',[
      job_ids
    ]);
