...
async function getPeriodAndUser(userId) {
  ...
  const calendarEnd = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999).toISOString();
  let user = null;
  try {
    const clerk = createClerkClient({ secretKey: process.env.CLERK_SECRET_KEY });
    user = await clerk.users.getUser(userId);
    const stripe = user?.privateMetadata?.stripe?.subscription;
    if (stripe?.current_period_start != null && stripe?.current_period_end != null) {
      return {
    ...
    if (clerk.billing?.getUserBillingSubscription) {
      ...
      if (item?.periodStart != null) {
        ...
        const end = item.periodEnd != null ? new Date(item.periodEnd) : new Date(start);
...
export const POST = async (request) => {
  ...
  if (!freelancer?.id) {
    return Response.json({ error: "Freelancer profile required" }, { status: 403 });
  }
  ...
  if ((countRes.rows[0]?.n ?? 0) === 0) {
    await db.query(`UPDATE fm_freelancers SET proposals_made = 0 WHERE clerk_id = $1`, [userId]);
  ...
  const content = JSON.stringify({ type: "proposal", jobId, text: String(message) });
